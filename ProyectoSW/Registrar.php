<html>
  ...
  <div>
    ...
    $soapclient2 = new nusoap_client('http://localhost:8080/ProyectoSW/ComprobarContrasena.php?wsdl', true);
    if(isset($_POST['direc'])){
      $mail = $_POST['direc'];
      $vip = $soapclient->call('comprobar', array('x'=>$mail));
      if($vip=="SI"){
        $contra = $_POST['pass'];
        $valida = $soapclient2->call('comprobar', array('x'=>$contra, 'z'=>1010));
        if($valida=="VALIDA"){
          ...
          echo ("<script>alert('La contraseña introducida es muy común y fácilmente descifrable; por favor utilice otra contraseña.')</script>");
        }else if($valida=="SIN SERVICIO"){
          echo ("<script>alert('Por favor, pague nuestra cuota para poder hacer uso de nuestro Servicio Web.')</script>");
        }
      }else if($vip=="NO"){
        echo ("<script>alert('El usuario introducido no es VIP, inténtelo con un usuario que sea VIP')</script>");
      }
    }
    ?>
  </div>
